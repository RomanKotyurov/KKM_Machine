...
def productRegistration(nameItem, price, quantity, markingCode, validationResult):
        ...
        fptr.setParam(1212, 33) # товар с кодом маркировки
        fptr.setParam(1214, 4)  # способ расчета - полный расчет
        fptr.setParam(IFptr.LIBFPTR_PARAM_MARKING_CODE, markingCode)
...
while True:      
        if newCheck:
            ...
            print('Общее количество новых чеков в базе:', totalNewCheck)          
            cur.execute('SELECT uuid, status, type, operatorName, clientInfoEmailOrPhone, paymentsType, paymentsSum, itemsQuantity  FROM "check" WHERE status = ?', (0,))   # Выбираем "непробитые" чеки
            resultsCheck = cur.fetchall()   # выборка "непробитых" чеков
            for rowCheck in resultsCheck:   # конкретный чек
                print("uuid: " + rowCheck[0])

                connectStatus, fptr = initializationKKT()   # инициализация и подключение ККТ  
                if connectStatus == 1:      # ККМ готова  
                    fptr.setParam(1021, rowCheck[3])
                    fptr.operatorLogin()
                    if rowCheck[2] == "sell":
                        fptr.setParam(IFptr.LIBFPTR_PARAM_RECEIPT_TYPE, IFptr.LIBFPTR_RT_SELL)  # LIBFPTR_RT_SELL - это продажа
                    ...
                    cur.execute('SELECT uuid, nameItem, price, quantity, imc FROM "item" WHERE uuid = ?', (rowCheck[0],))
                    resultsItem = cur.fetchall()    # выборка товаров в чеке из выборки
                    for rowItem in resultsItem:     # для каждого товара в выбранном чеке
                        print(rowItem)   
                        markingCode, validationResult = checkOfMarring(rowItem[4])      # проверка КМ товара
                        productRegistration(rowItem[1], rowItem[2], rowItem[3], markingCode, validationResult)      # регистрация товара в чеке     
                    if rowCheck[5] == "electronically":
                    ...
                    fptr.setParam(IFptr.LIBFPTR_PARAM_PAYMENT_SUM, rowCheck[6])
                    fptr.payment()
                    fptr.closeReceipt()     # закрытие чека
                    CheckClosed = checkReceiptClosed()    # обработка результата операции
                    if CheckClosed:
                        cur.execute('UPDATE "check" SET status = ? WHERE uuid = ?', (1, rowCheck[0]))   # смена статуса чека
                        botMessage = "Обработан на ККТ чек на сумму " + str(rowCheck[6]) + " руб."
                        #bot.send_message(user_id, botMessage)
                        totalNewCheck -= 1
                fptr.close()
            con.commit()
...
def initializationKKT():
    fptr = IFptr("")    # инициализация драйвера
    #version = fptr.version()
    #print('driver version:  ' + str(version))
    settings = {    # подключение ККТ
        IFptr.LIBFPTR_SETTING_MODEL: IFptr.LIBFPTR_MODEL_ATOL_AUTO,
        IFptr.LIBFPTR_SETTING_PORT: IFptr.LIBFPTR_PORT_TCPIP,
        IFptr.LIBFPTR_SETTING_IPADDRESS: KASSA_IP, # 153, 154
        IFptr.LIBFPTR_SETTING_IPPORT: 5555
    }
    fptr.setSettings(settings)
    fptr.open()
